                            ...
                            <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap"
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($user['status'] == '0')
                                                <button class="change_status btn-sm btn-danger"
                                                    data-id="{{ $user['id'] }}" data-status="1">Inactive</button>
                                            @else
                                                <button class="change_status btn-sm btn-success"
                                                    data-id="{{ $user['id'] }}" data-status="0">Active</button>
                                            @endif
                                            <a href="javascript:void(0);" onclick="deleteuser('{{ $user['id'] }}')"
                                                class="btn-sm"><i class="bx bx-trash-alt"></i></a>
                                            <a href="{{ url('profile/' . $user['id']) }}" target="_blank"
                                                class="btn-sm">login</a>
                                            @if ($pagename == 'Manage Club')
                                                <a href="{{ url('offers/' . $user['id']) }}" target="_blank"
                                                    class="btn-sm">offers</a>
                                            @endif
                                        </td>
                                    </tr>
                                    <?php $i++;
